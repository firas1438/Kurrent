...
const getTaskSummary = async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ message: "Failed to fetch task summary statistics" });
  }
};
...
const getTaskCompletionTrend = async (req, res) => {
  try {
    ...
    const data = Array.from(byDate.entries()).map(([date, completed]) => ({
      date,
      completed,
    }));
    ...
    res
      .status(500)
      .json({ message: "Failed to fetch task completion trend" });
  }
};
...
const getTaskCreationTrend = async (req, res) => {
  try {
    ...
    const data = Array.from(byDate.entries()).map(([date, created]) => ({
      date,
      created,
    }));
